...
* Optionally triggers an interrupt to inform main controller of freed buffer
...
For the transmission of RPMsgs from the remote to the main controller the
following actions are performed on VRING0, as detailed in the lower part
of the flow chart above.

The remote

* Gets a transmission buffer, provided by main controller, from the available queue of VRING0.
* Writes the RPMsg header and payload of this buffer.
...
buffer provider. The main controller is in charge of the shared memory allocation.
